...
- Feast integration for historical retrieval
...
ML training pipeline with historical retrieval.
...
- Behavioral mode with persistent per-user activity weights (Day-9 addition)
...
| Feature platform | Feast | Feature definitions, historical retrieval, materialization |
...
3. Implement Redis materialization and online feature lookup.
4. Add online/offline parity tests.
5. Add quality gates before materialization.
6. Add freshness checks, richer run manifests, failure simulations, and runbooks.
7. Document the AWS S3 and DynamoDB production profile.
8. Add GitHub Actions CI and a one-command demo.
9. Implement hyperparameter tuning and advanced models (XGBoost, LightGBM).
